# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: mercury.proto

from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)




DESCRIPTOR = _descriptor.FileDescriptor(
  name='mercury.proto',
  package='mercury',
  serialized_pb='\n\rmercury.proto\x12\x07mercury\"\xf1\x05\n\x0eMercuryMessage\x12\x0c\n\x04uuid\x18\x01 \x01(\t\x12-\n\x04type\x18\x02 \x01(\x0e\x32\x1f.mercury.MercuryMessage.MsgType\x12\x31\n\x08src_addr\x18\x03 \x01(\x0b\x32\x1f.mercury.MercuryMessage.Address\x12\x31\n\x08\x64st_addr\x18\x04 \x01(\x0b\x32\x1f.mercury.MercuryMessage.Address\x12(\n\x0bsession_msg\x18\x05 \x01(\x0b\x32\x13.mercury.SessionMsg\x12&\n\npubsub_msg\x18\x06 \x01(\x0b\x32\x12.mercury.PubSubMsg\x12&\n\nappcli_msg\x18\x07 \x01(\x0b\x32\x12.mercury.AppCliMsg\x1aw\n\x07GeoAddr\x12\x0c\n\x04type\x18\x01 \x01(\t\x12:\n\nattributes\x18\x02 \x03(\x0b\x32&.mercury.MercuryMessage.GeoAddr.KeyVal\x1a\"\n\x06KeyVal\x12\x0b\n\x03key\x18\x01 \x01(\t\x12\x0b\n\x03val\x18\x02 \x01(\t\x1a\x8c\x01\n\x07\x41\x64\x64ress\x12.\n\x04type\x18\x01 \x01(\x0e\x32 .mercury.MercuryMessage.AddrType\x12\x0e\n\x06\x63li_id\x18\x02 \x01(\x03\x12\x31\n\x08geo_addr\x18\x03 \x01(\x0b\x32\x1f.mercury.MercuryMessage.GeoAddr\x12\x0e\n\x06\x61pp_id\x18\x04 \x01(\x03\"w\n\x07MsgType\x12\x0b\n\x07\x41\x44_SESS\x10\x01\x12\x0b\n\x07PUB_CLI\x10\x02\x12\x0c\n\x08\x43LI_SESS\x10\x03\x12\x0e\n\nCLI_SUBSCR\x10\x04\x12\r\n\tCLI_UNSUB\x10\x05\x12\x0b\n\x07\x43LI_PUB\x10\x06\x12\x0b\n\x07\x43LI_APP\x10\x07\x12\x0b\n\x07\x41PP_CLI\x10\x08\"A\n\x08\x41\x64\x64rType\x12\n\n\x06\x43LIENT\x10\x01\x12\x0b\n\x07\x41\x44\x41PTER\x10\x02\x12\n\n\x06PUBSUB\x10\x03\x12\x07\n\x03GEO\x10\x04\x12\x07\n\x03\x41PP\x10\x05\"\xd5\x01\n\nSessionMsg\x12\n\n\x02id\x18\x01 \x01(\x05\x12)\n\x04type\x18\x02 \x01(\x0e\x32\x1b.mercury.SessionMsg.MsgType\x12.\n\nattributes\x18\x03 \x03(\x0b\x32\x1a.mercury.SessionMsg.KeyVal\x1a\"\n\x06KeyVal\x12\x0b\n\x03key\x18\x01 \x01(\t\x12\x0b\n\x03val\x18\x02 \x01(\t\"<\n\x07MsgType\x12\x08\n\x04INIT\x10\x01\x12\t\n\x05\x43LOSE\x10\x02\x12\n\n\x06\x43LIREP\x10\x03\x12\x06\n\x02HB\x10\x04\x12\x08\n\x04\x45\x43HO\x10\x05\"m\n\tPubSubMsg\x12\r\n\x05topic\x18\x01 \x01(\t\x12-\n\nattributes\x18\x02 \x03(\x0b\x32\x19.mercury.PubSubMsg.KeyVal\x1a\"\n\x06KeyVal\x12\x0b\n\x03key\x18\x01 \x01(\t\x12\x0b\n\x03val\x18\x02 \x01(\t\"l\n\tAppCliMsg\x12\x0c\n\x04type\x18\x01 \x01(\t\x12-\n\nattributes\x18\x02 \x03(\x0b\x32\x19.mercury.AppCliMsg.KeyVal\x1a\"\n\x06KeyVal\x12\x0b\n\x03key\x18\x01 \x01(\t\x12\x0b\n\x03val\x18\x02 \x01(\t')



_MERCURYMESSAGE_MSGTYPE = _descriptor.EnumDescriptor(
  name='MsgType',
  full_name='mercury.MercuryMessage.MsgType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='AD_SESS', index=0, number=1,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PUB_CLI', index=1, number=2,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLI_SESS', index=2, number=3,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLI_SUBSCR', index=3, number=4,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLI_UNSUB', index=4, number=5,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLI_PUB', index=5, number=6,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLI_APP', index=6, number=7,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='APP_CLI', index=7, number=8,
      options=None,
      type=None),
  ],
  containing_type=None,
  options=None,
  serialized_start=594,
  serialized_end=713,
)

_MERCURYMESSAGE_ADDRTYPE = _descriptor.EnumDescriptor(
  name='AddrType',
  full_name='mercury.MercuryMessage.AddrType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='CLIENT', index=0, number=1,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ADAPTER', index=1, number=2,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='PUBSUB', index=2, number=3,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='GEO', index=3, number=4,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='APP', index=4, number=5,
      options=None,
      type=None),
  ],
  containing_type=None,
  options=None,
  serialized_start=715,
  serialized_end=780,
)

_SESSIONMSG_MSGTYPE = _descriptor.EnumDescriptor(
  name='MsgType',
  full_name='mercury.SessionMsg.MsgType',
  filename=None,
  file=DESCRIPTOR,
  values=[
    _descriptor.EnumValueDescriptor(
      name='INIT', index=0, number=1,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLOSE', index=1, number=2,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='CLIREP', index=2, number=3,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='HB', index=3, number=4,
      options=None,
      type=None),
    _descriptor.EnumValueDescriptor(
      name='ECHO', index=4, number=5,
      options=None,
      type=None),
  ],
  containing_type=None,
  options=None,
  serialized_start=936,
  serialized_end=996,
)


_MERCURYMESSAGE_GEOADDR_KEYVAL = _descriptor.Descriptor(
  name='KeyVal',
  full_name='mercury.MercuryMessage.GeoAddr.KeyVal',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='key', full_name='mercury.MercuryMessage.GeoAddr.KeyVal.key', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='val', full_name='mercury.MercuryMessage.GeoAddr.KeyVal.val', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=415,
  serialized_end=449,
)

_MERCURYMESSAGE_GEOADDR = _descriptor.Descriptor(
  name='GeoAddr',
  full_name='mercury.MercuryMessage.GeoAddr',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='type', full_name='mercury.MercuryMessage.GeoAddr.type', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='attributes', full_name='mercury.MercuryMessage.GeoAddr.attributes', index=1,
      number=2, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[_MERCURYMESSAGE_GEOADDR_KEYVAL, ],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=330,
  serialized_end=449,
)

_MERCURYMESSAGE_ADDRESS = _descriptor.Descriptor(
  name='Address',
  full_name='mercury.MercuryMessage.Address',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='type', full_name='mercury.MercuryMessage.Address.type', index=0,
      number=1, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=1,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='cli_id', full_name='mercury.MercuryMessage.Address.cli_id', index=1,
      number=2, type=3, cpp_type=2, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='geo_addr', full_name='mercury.MercuryMessage.Address.geo_addr', index=2,
      number=3, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='app_id', full_name='mercury.MercuryMessage.Address.app_id', index=3,
      number=4, type=3, cpp_type=2, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=452,
  serialized_end=592,
)

_MERCURYMESSAGE = _descriptor.Descriptor(
  name='MercuryMessage',
  full_name='mercury.MercuryMessage',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='uuid', full_name='mercury.MercuryMessage.uuid', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='type', full_name='mercury.MercuryMessage.type', index=1,
      number=2, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=1,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='src_addr', full_name='mercury.MercuryMessage.src_addr', index=2,
      number=3, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='dst_addr', full_name='mercury.MercuryMessage.dst_addr', index=3,
      number=4, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='session_msg', full_name='mercury.MercuryMessage.session_msg', index=4,
      number=5, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='pubsub_msg', full_name='mercury.MercuryMessage.pubsub_msg', index=5,
      number=6, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='appcli_msg', full_name='mercury.MercuryMessage.appcli_msg', index=6,
      number=7, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[_MERCURYMESSAGE_GEOADDR, _MERCURYMESSAGE_ADDRESS, ],
  enum_types=[
    _MERCURYMESSAGE_MSGTYPE,
    _MERCURYMESSAGE_ADDRTYPE,
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=27,
  serialized_end=780,
)


_SESSIONMSG_KEYVAL = _descriptor.Descriptor(
  name='KeyVal',
  full_name='mercury.SessionMsg.KeyVal',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='key', full_name='mercury.SessionMsg.KeyVal.key', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='val', full_name='mercury.SessionMsg.KeyVal.val', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=415,
  serialized_end=449,
)

_SESSIONMSG = _descriptor.Descriptor(
  name='SessionMsg',
  full_name='mercury.SessionMsg',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='id', full_name='mercury.SessionMsg.id', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='type', full_name='mercury.SessionMsg.type', index=1,
      number=2, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=1,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='attributes', full_name='mercury.SessionMsg.attributes', index=2,
      number=3, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[_SESSIONMSG_KEYVAL, ],
  enum_types=[
    _SESSIONMSG_MSGTYPE,
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=783,
  serialized_end=996,
)


_PUBSUBMSG_KEYVAL = _descriptor.Descriptor(
  name='KeyVal',
  full_name='mercury.PubSubMsg.KeyVal',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='key', full_name='mercury.PubSubMsg.KeyVal.key', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='val', full_name='mercury.PubSubMsg.KeyVal.val', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=415,
  serialized_end=449,
)

_PUBSUBMSG = _descriptor.Descriptor(
  name='PubSubMsg',
  full_name='mercury.PubSubMsg',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='topic', full_name='mercury.PubSubMsg.topic', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='attributes', full_name='mercury.PubSubMsg.attributes', index=1,
      number=2, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[_PUBSUBMSG_KEYVAL, ],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=998,
  serialized_end=1107,
)


_APPCLIMSG_KEYVAL = _descriptor.Descriptor(
  name='KeyVal',
  full_name='mercury.AppCliMsg.KeyVal',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='key', full_name='mercury.AppCliMsg.KeyVal.key', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='val', full_name='mercury.AppCliMsg.KeyVal.val', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=415,
  serialized_end=449,
)

_APPCLIMSG = _descriptor.Descriptor(
  name='AppCliMsg',
  full_name='mercury.AppCliMsg',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='type', full_name='mercury.AppCliMsg.type', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=unicode("", "utf-8"),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
    _descriptor.FieldDescriptor(
      name='attributes', full_name='mercury.AppCliMsg.attributes', index=1,
      number=2, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None),
  ],
  extensions=[
  ],
  nested_types=[_APPCLIMSG_KEYVAL, ],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  extension_ranges=[],
  serialized_start=1109,
  serialized_end=1217,
)

_MERCURYMESSAGE_GEOADDR_KEYVAL.containing_type = _MERCURYMESSAGE_GEOADDR;
_MERCURYMESSAGE_GEOADDR.fields_by_name['attributes'].message_type = _MERCURYMESSAGE_GEOADDR_KEYVAL
_MERCURYMESSAGE_GEOADDR.containing_type = _MERCURYMESSAGE;
_MERCURYMESSAGE_ADDRESS.fields_by_name['type'].enum_type = _MERCURYMESSAGE_ADDRTYPE
_MERCURYMESSAGE_ADDRESS.fields_by_name['geo_addr'].message_type = _MERCURYMESSAGE_GEOADDR
_MERCURYMESSAGE_ADDRESS.containing_type = _MERCURYMESSAGE;
_MERCURYMESSAGE.fields_by_name['type'].enum_type = _MERCURYMESSAGE_MSGTYPE
_MERCURYMESSAGE.fields_by_name['src_addr'].message_type = _MERCURYMESSAGE_ADDRESS
_MERCURYMESSAGE.fields_by_name['dst_addr'].message_type = _MERCURYMESSAGE_ADDRESS
_MERCURYMESSAGE.fields_by_name['session_msg'].message_type = _SESSIONMSG
_MERCURYMESSAGE.fields_by_name['pubsub_msg'].message_type = _PUBSUBMSG
_MERCURYMESSAGE.fields_by_name['appcli_msg'].message_type = _APPCLIMSG
_MERCURYMESSAGE_MSGTYPE.containing_type = _MERCURYMESSAGE;
_MERCURYMESSAGE_ADDRTYPE.containing_type = _MERCURYMESSAGE;
_SESSIONMSG_KEYVAL.containing_type = _SESSIONMSG;
_SESSIONMSG.fields_by_name['type'].enum_type = _SESSIONMSG_MSGTYPE
_SESSIONMSG.fields_by_name['attributes'].message_type = _SESSIONMSG_KEYVAL
_SESSIONMSG_MSGTYPE.containing_type = _SESSIONMSG;
_PUBSUBMSG_KEYVAL.containing_type = _PUBSUBMSG;
_PUBSUBMSG.fields_by_name['attributes'].message_type = _PUBSUBMSG_KEYVAL
_APPCLIMSG_KEYVAL.containing_type = _APPCLIMSG;
_APPCLIMSG.fields_by_name['attributes'].message_type = _APPCLIMSG_KEYVAL
DESCRIPTOR.message_types_by_name['MercuryMessage'] = _MERCURYMESSAGE
DESCRIPTOR.message_types_by_name['SessionMsg'] = _SESSIONMSG
DESCRIPTOR.message_types_by_name['PubSubMsg'] = _PUBSUBMSG
DESCRIPTOR.message_types_by_name['AppCliMsg'] = _APPCLIMSG

class MercuryMessage(_message.Message):
  __metaclass__ = _reflection.GeneratedProtocolMessageType

  class GeoAddr(_message.Message):
    __metaclass__ = _reflection.GeneratedProtocolMessageType

    class KeyVal(_message.Message):
      __metaclass__ = _reflection.GeneratedProtocolMessageType
      DESCRIPTOR = _MERCURYMESSAGE_GEOADDR_KEYVAL

      # @@protoc_insertion_point(class_scope:mercury.MercuryMessage.GeoAddr.KeyVal)
    DESCRIPTOR = _MERCURYMESSAGE_GEOADDR

    # @@protoc_insertion_point(class_scope:mercury.MercuryMessage.GeoAddr)

  class Address(_message.Message):
    __metaclass__ = _reflection.GeneratedProtocolMessageType
    DESCRIPTOR = _MERCURYMESSAGE_ADDRESS

    # @@protoc_insertion_point(class_scope:mercury.MercuryMessage.Address)
  DESCRIPTOR = _MERCURYMESSAGE

  # @@protoc_insertion_point(class_scope:mercury.MercuryMessage)

class SessionMsg(_message.Message):
  __metaclass__ = _reflection.GeneratedProtocolMessageType

  class KeyVal(_message.Message):
    __metaclass__ = _reflection.GeneratedProtocolMessageType
    DESCRIPTOR = _SESSIONMSG_KEYVAL

    # @@protoc_insertion_point(class_scope:mercury.SessionMsg.KeyVal)
  DESCRIPTOR = _SESSIONMSG

  # @@protoc_insertion_point(class_scope:mercury.SessionMsg)

class PubSubMsg(_message.Message):
  __metaclass__ = _reflection.GeneratedProtocolMessageType

  class KeyVal(_message.Message):
    __metaclass__ = _reflection.GeneratedProtocolMessageType
    DESCRIPTOR = _PUBSUBMSG_KEYVAL

    # @@protoc_insertion_point(class_scope:mercury.PubSubMsg.KeyVal)
  DESCRIPTOR = _PUBSUBMSG

  # @@protoc_insertion_point(class_scope:mercury.PubSubMsg)

class AppCliMsg(_message.Message):
  __metaclass__ = _reflection.GeneratedProtocolMessageType

  class KeyVal(_message.Message):
    __metaclass__ = _reflection.GeneratedProtocolMessageType
    DESCRIPTOR = _APPCLIMSG_KEYVAL

    # @@protoc_insertion_point(class_scope:mercury.AppCliMsg.KeyVal)
  DESCRIPTOR = _APPCLIMSG

  # @@protoc_insertion_point(class_scope:mercury.AppCliMsg)


# @@protoc_insertion_point(module_scope)
